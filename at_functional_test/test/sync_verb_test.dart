...
void main() {
  ...
  test('sync verb ', () async {
   ...
   test('sync verb with regex ', () async {
    /// UPDATE VERB
    await socket_writer(_socket_first_atsign, 'update:public:twitter.persona$first_atsign bob_tweet');
    var response = await read();
    print('update verb response : $response');
    assert((!response.contains('Invalid syntax')) && (!response.contains('null')));
    var commitId = response.replaceAll('data:','');
    ...
    response = await read();
    print('sync response is : $response');
    assert((response.contains('"atKey":"public:twitter$regex$first_atsign')) && (!response.contains('"atKey":"public:location$first_atsign')));
  });
